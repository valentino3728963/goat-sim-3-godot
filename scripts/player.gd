...
func _mesh_normal() -> void:
	var goat_tan: Color = Color(0.85, 0.80, 0.68)
	var dark: Color     = Color(0.40, 0.35, 0.25)
	var horn: Color     = Color(0.55, 0.48, 0.28)
	var _hoof: Color    = Color(0.15, 0.12, 0.10)
	var eye_c: Color    = Color(0.08, 0.06, 0.04)
	_mbox(goat_root, Vector3(0, 1.00, 0),    Vector3(0.70, 0.45, 1.10), goat_tan,  0.80, 0.00)
	_mcyl(goat_root, Vector3(0, 1.38, 0.40), 0.12, 0.13, 0.35, goat_tan,  0.80, 0.00, Vector3(-0.4, 0, 0))
	_mbox(goat_root, Vector3(0, 1.56, 0.72), Vector3(0.38, 0.34, 0.44), goat_tan,  0.80, 0.00)
	...
	for sx in [-1, 1]:
		_mbox(goat_root, Vector3(sx * 0.22, 1.73, 0.65), Vector3(0.08, 0.16, 0.06), goat_tan,
	...
	for i in 4:
		...
		_mcyl(goat_root, Vector3(bx, 0.62, bz), 0.08, 0.09, 0.32, goat_tan,  0.80, 0.00)
		_mcyl(goat_root, Vector3(bx, 0.28, bz), 0.06, 0.07, 0.32, dark, 0.70, 0.00)
		_mbox(goat_root, Vector3(bx, 0.06, bz + 0.04), Vector3(0.10, 0.08, 0.14), _hoof, 1.00, 0.00)

func _mesh_tall() -> void:
	var goat_tan: Color = Color(0.92, 0.85, 0.55)
	var dark: Color     = Color(0.50, 0.40, 0.20)
	var horn: Color     = Color(0.55, 0.48, 0.28)
	var _hoof: Color    = Color(0.15, 0.12, 0.10)
	_mbox(goat_root, Vector3(0, 1.20, 0),    Vector3(0.65, 0.42, 1.00), goat_tan, 0.80, 0.00)
	_mcyl(goat_root, Vector3(0, 2.20, 0.35), 0.10, 0.12, 1.20, goat_tan, 0.80, 0.00, Vector3(-0.3, 0, 0))
	_mbox(goat_root, Vector3(0, 2.90, 0.65), Vector3(0.36, 0.32, 0.42), goat_tan, 0.80, 0.00)
	...
	for i in 4:
		var bx: float = leg_x[i]
		var bz: float = leg_z[i]
		_mcyl(goat_root, Vector3(bx, 0.80, bz), 0.07, 0.08, 0.60, goat_tan,  0.80, 0.00)
		_mcyl(goat_root, Vector3(bx, 0.32, bz), 0.05, 0.06, 0.50, dark, 0.70, 0.00)
		_mbox(goat_root, Vector3(bx, 0.04, bz + 0.04), Vector3(0.10, 0.08, 0.14), _hoof, 1.00, 0.00)

func _mesh_muscle() -> void:
	var c: Color     = Color(0.55, 0.22, 0.14)
	var dark: Color  = Color(0.30, 0.12, 0.08)
	var _hoof: Color = Color(0.15, 0.12, 0.10)
	...
	for i in 4:
		...
		_mbox(goat_root, Vector3(bx, 0.04, bz + 0.04), Vector3(0.14, 0.08, 0.16), _hoof, 1.00, 0.00)
	for sx in [-1, 1]:
...
func _mesh_speed() -> void:
	var c: Color     = Color(0.20, 0.60, 0.90)
	var dark: Color  = Color(0.10, 0.35, 0.60)
	var _hoof: Color = Color(0.08, 0.08, 0.10)
	...
	for i in 4:
		...
		_mbox(goat_root, Vector3(bx, 0.04, bz + 0.04), Vector3(0.08, 0.07, 0.12), _hoof, 1.00, 0.00)
...
func _mesh_giant() -> void:
	var c: Color     = Color(0.40, 0.65, 0.30)
	var dark: Color  = Color(0.22, 0.40, 0.14)
	var horn: Color  = Color(0.62, 0.54, 0.28)
	var _hoof: Color = Color(0.15, 0.12, 0.10)
...
func _mesh_ghost() -> void:
	var c: Color     = Color(0.80, 0.85, 1.0, 0.50)
	var dark: Color  = Color(0.55, 0.60, 0.80, 0.50)
	var _hoof: Color = Color(0.40, 0.45, 0.70, 0.50)
	...
	for i in 4:
		...
		_mbox(goat_root, Vector3(bx, 0.06, bz + 0.04), Vector3(0.10, 0.08, 0.14), _hoof, 0.80, 0.00)
	_apply_alpha_to_mesh(goat_root, 0.50)

func _mesh_bouncy() -> void:
	var c: Color     = Color(1.0, 0.45, 0.80)
	var dark: Color  = Color(0.70, 0.20, 0.55)
	var _hoof: Color = Color(0.40, 0.10, 0.28)
...
func _physics_process(delta: float) -> void:
	...
	if idir.length_squared() > 0.0:
		...
		var cam_basis: Basis = cam_pivot.global_transform.basis
		var fwd: Vector3   = -cam_basis.z; fwd.y = 0.0; fwd = fwd.normalized()
		var right: Vector3 =  cam_basis.x; right.y = 0.0; right = right.normalized()
